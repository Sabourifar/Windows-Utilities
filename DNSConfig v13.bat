...
for /f "delims=" %%a in ('curl -s --max-time 2 https://icanhazip.com 2^>nul') do set "public_ip=%%a"
if "!public_ip!"=="Not available" (
    for /f "delims=" %%a in ('curl -s --max-time 2 https://api.ipify.org 2^>nul') do set "public_ip=%%a"
)
exit /b
